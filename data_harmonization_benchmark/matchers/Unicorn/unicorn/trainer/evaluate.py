...
def evaluate_moe(
    ...
    if write:
        ...
        with open(
            dataformat.deepmatcher_data[args.datasetkey]
            + str(args.labelnum)
            + "selecttrain.json",
            "r",
            encoding="utf8",
        ...
        remainselect = [
            waitselect[i] for i in range(len(waitselect)) if i not in select
        ]
        newselect = alreadyselect + addselect
        with open(
            dataformat.deepmatcher_data[args.datasetkey]
            + str(args.labelnum + 200)
            + "selecttrain.json",
            "w",
        ) as f:
